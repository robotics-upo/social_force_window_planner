...
double getGoalPositionDistance(const geometry_msgs::PoseStamped &global_pose,
...
double
getGoalOrientationAngleDifference(const geometry_msgs::PoseStamped &global_pose,
                                  double goal_th) {
...
bool getGoalPose(const tf2_ros::Buffer &tf,
  ...
  try {
    geometry_msgs::TransformStamped transform = tf.lookupTransform(
        global_frame, ros::Time(), plan_goal_pose.header.frame_id,
...
bool isGoalReached(const tf2_ros::Buffer &tf,
  ...
  if (getGoalPositionDistance(global_pose, goal_x, goal_y) <=
      xy_goal_tolerance) {
    // check to see if the goal orientation has been reached
    if (fabs(getGoalOrientationAngleDifference(global_pose, goal_th)) <=
        yaw_goal_tolerance) {
